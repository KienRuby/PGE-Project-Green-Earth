...
public sealed class ChipManager : MonoBehaviour
{
    ...
    static ChipManager()
    {
        ...
        PlayerDataService.OnChipsetBoxesChanged += val =>
        {
            OnChipsetBoxesChanged?.Invoke(val);
        };
        PlayerDataService.OnDroneBoxesChanged += val =>
        {
            OnDroneBoxesChanged?.Invoke(val);
        };
    }
    ...
    {
        get => PlayerDataService.ChipsetBoxes;
        set => PlayerDataService.ChipsetBoxes = value;
    }

    public static int DroneBoxes
    {
        get => PlayerDataService.DroneBoxes;
        set => PlayerDataService.DroneBoxes = value;
    }
    ...
    public static bool HasEnoughChipsetBoxes(int amount)
    {
        if (amount < 0) return false;
        return ChipsetBoxes >= amount;
    }

    public static bool HasEnoughDroneBoxes(int amount)
    {
        if (amount < 0) return false;
        return DroneBoxes >= amount;
    }
    ...
    {
        if (amount < 0) return false;
        return PlayerDataService.TrySpendChipsetBoxes(amount);
    }

    public static bool TrySpendDroneBoxes(int amount)
    {
        if (amount < 0) return false;
        return PlayerDataService.TrySpendDroneBoxes(amount);
    }
    ...
    public void NotifyAllBalancesChanged()
    {
        ...
        OnChipsetBoxesChanged?.Invoke(ChipsetBoxes);
        OnDroneBoxesChanged?.Invoke(DroneBoxes);
    }
