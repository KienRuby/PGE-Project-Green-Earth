            ...
            try
            {
                ...
                if (PlayerPrefs.HasKey(key))
                {
                    ...
                    if (!string.IsNullOrEmpty(json))
                    {
                        ...
                        if (cloudData != null)
                        {
                            // This backend is a local, balance-only snapshot, not a complete cloud save.
                            // Restoring any old balance would retain purchased upgrades and refund their cost.
                            onComplete?.Invoke(false, "Bản sao cũ chỉ chứa số dư, không thể khôi phục tiến trình an toàn. Dữ liệu hiện tại được giữ nguyên.");
                            return;
                        }
                    }
                }
