...
public static class PlayerDataService
{
    ...
    // =========================================================================
    // BUDDY / DRONE DECK PERSISTENCE (3 Slots per Deck x 3 Preset Decks)
    // =========================================================================
    public const string BuddyActiveDeckKey = "PGE.Buddy.ActiveDeck";
    public const string BuddyDeckPrefix = "PGE.Buddy.Deck.";
    ...
    public static string GetBuddyDeckKey(int deckIndex) => $"{BuddyDeckPrefix}{deckIndex}";

    public static void SaveBuddyDeck(int deckIndex, int[] equippedIds)
    {
        if (equippedIds == null) return;
        string serialized = string.Join(",", equippedIds);
        PlayerPrefs.SetString(GetBuddyDeckKey(deckIndex), serialized);
        PlayerPrefs.Save();
    }

    public static int[] LoadBuddyDeck(int deckIndex, int[] defaultIds = null)
    {
        string key = GetBuddyDeckKey(deckIndex);
        if (!PlayerPrefs.HasKey(key))
        {
            return defaultIds != null ? (int[])defaultIds.Clone() : new int[] { -1, -1, -1 };
        }
